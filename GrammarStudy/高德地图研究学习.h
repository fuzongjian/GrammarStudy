...
//  GrammarStudy
//
//  Created by 陈舒澳 on 16/5/10.
//  Copyright © 2016年 speeda. All rights reserved.
//

#ifndef _______________h
#define _______________h

#pragma mark --- 调起手机高德地图  配置白名单
/*
...
/**
 ...
 @property CLLocationDistance altitude;  //海拔高度。正数表示在海平面之上，而负数表示在海平面之下。
 @property CLLocationAccuracy horizontalAccuracy; 位置的精度(半径)。位置精度通过一个圆表示，实际位置可能位于这个圆内的任何地方。这个圆是由coordinate(坐标)和horizontalAccuracy(半径)共同决定的，horizontalAccuracy的值越大，那么定义的圆就越大，因此位置精度就越低。如果horizontalAccuracy的值为负，则表明coordinate的值无效
 @property CLLocationAccuracy verticalAccuracy; 海拔高度的精度。为正值表示海拔高度的误差为对应的米数；为负表示altitude(海拔高度)的值无效。
 @property CLLocationDirection course; //方向
 @property CLLocationSpeed speed; //该属性是通过比较当前位置和前一个位置，并比较它们之间的时间差异和距离计算得到的。鉴于Core Location更新的频率，speed属性的值不是非常精确，除非移动速度变化很小。
 -（NSDate *）timeStamp;
 */
